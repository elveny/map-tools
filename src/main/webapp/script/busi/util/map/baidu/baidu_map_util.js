...
var MAP_TOOLS = {
	...
	initMap : function(params){
		// 创建Map实例
		var map = new BMap.Map(params.domId);
		// 初始化地图,设置中心点坐标和地图级别
		map.centerAndZoom(new BMap.Point(params.center.longitude, params.center.latitude), params.zoom);
		
		// 设置地图级数范围
		if(params.zoomRange){
			if(params.zoomRange.minZoom){
				map.setMinZoom(params.zoomRange.minZoom);
			}
			
			if(params.zoomRange.maxZoom){
				map.setMaxZoom(params.zoomRange.maxZoom);
			}
		}
		...
		if(params.control == null){
			...
			if(params.control.navigation == null || params.control.navigation){
				map.addControl(new BMap.NavigationControl({anchor:BMAP_ANCHOR_TOP_RIGHT }));               // 添加平移缩放控件
			}
			if(params.control.scale == null || params.control.scale){
				map.addControl(new BMap.ScaleControl());                    // 添加比例尺控件
			}
			if(params.control.overviewMap == null || params.control.overviewMap){
				map.addControl(new BMap.OverviewMapControl({isOpen:true}));              //添加缩略地图控件
			}
			if(params.control.mapType == null || params.control.mapType){
				map.addControl(new BMap.MapTypeControl({anchor: BMAP_ANCHOR_TOP_LEFT, mapTypes: [BMAP_NORMAL_MAP,BMAP_HYBRID_MAP]}));     //2D图，卫星图
			}
		}
		
		//启用滚轮放大缩小
		if(params.scrollWheelZoom == null || params.scrollWheelZoom){
...
//					icon.setAnchor(new BMap.Size(params.icon.anchor.x, params.icon.anchor.y));
		...
		if(params.infoWindow != null){
			var infoWindow = new BMap.InfoWindow(params.infoWindow.content, params.infoWindow.opts);
	...
	newCircle : function(params){
		...
		var center = new BMap.Point(params.center.longitude, params.center.latitude);
	...
	addOverlay : function(params){
		...
		if(params.viewPointsMap == null || params.viewPointsMap){
			...
			if(overlay.toString() == "[object Marker]"){
				...
				viewPointsMap.put(params.id, viewPoints);
			}
			
			//Polyline & Polygon
			if(overlay.toString() == "[object Polyline]" || overlay.toString() == "[object Polygon]"){
				viewPointsMap.put(params.id, overlay.getPath());
			}
	...
	removeOverlayById : function(params){
		...
		//将地图上指定编号的覆盖物移除
		MAP_TOOLS.removeOverlay({map: map, overlay:overalyMap.get(params.id)});
	...
	autoCenterAndZoom : function(params){
		...
		if(params != null && params.points != null){
			...
			for(var i in points){
				viewPoints.push(new BMap.Point(points[i].longitude, points[i].latitude));
			}
		...
		else if(!viewPointsMap.isEmpty()){
			
			var viewPoints = new Array();
			
			var values = viewPointsMap.values();
			for(var i in values){
				var points = values[i];
				for(var j in points){
					...
					driving.setSearchCompleteCallback(function(results){
						...
						if(driving.getStatus() == BMAP_STATUS_SUCCESS){
							...
							if(num < (points.length-1)){
								this.native_driving_route_recursion(map, driving, num+1, points, drivingRouteResultPoints, callback);
							}
							else{
